...
//        s.append(String.format(padRest + "s", ""));
        for(int i = 0; i < lengthCompound; i++){s.append(" ");};
        for(int i = 0; i < lengthRest; i++){s.append(" ");};
        
        for (C col : target.getAsList()) {
//            s.append(String.format(padRest + "s", compoundSet.getStringForCompound(col)));
            for(int i = 0; i < lengthRest-1; i++){s.append(" ");}
            s.append(compoundSet.getStringForCompound(col));
        }
...
//                    compoundSet.getStringForCompound(query.getCompoundAt(row))));
            if(row == 0){for(int i = 0; i < lengthCompound; i++){s.append(" ");};}
            else{
            	for(int i = 0; i < lengthCompound; i++){s.append(" ");}
            	s.append(compoundSet.getStringForCompound(query.getCompoundAt(row)));
            }
            for (int col = 0; col <= target.getLength(); col++) {
//            	  s.append(String.format(padRest + "d", getScoreMatrixAt(row, col)));
            	  for(int i = 0; i < lengthRest-1; i++){s.append(" ");}
                  s.append(getScoreMatrixAt(row, col));
            }
//            s.append(String.format("%n"));
              s.append("/n");
        }
